...
import { generateReengageEmail } from "@/lib/ai";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const content = await generateReengageEmail({ email, name, company });
    ...
    pushActivity({
      ...
      message: `Re-engagement email sent to ${email}`,
    ...
    return NextResponse.json({ error: "Failed to send re-engage" }, { status: 500 });
  }
}
